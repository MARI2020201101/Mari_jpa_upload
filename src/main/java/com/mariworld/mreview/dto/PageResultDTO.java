package com.mariworld.mreview.dto;


import lombok.Data;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;

import java.util.List;
import java.util.function.Function;
import java.util.stream.Collectors;
import java.util.stream.IntStream;

@Data
public class PageResultDTO<DTO, EN>{
    private List<DTO> dtoList;

    private int totalPage;

    private int size,page;
    private boolean prev,next;
    private int start,end;
    private int tempEnd;

    private List<Integer> pageList;

    public PageResultDTO(Page<EN>result, Function<EN,DTO> fn){
        this.totalPage = result.getTotalPages();
        Pageable pageRequest = result.getPageable();
        makePageResult(pageRequest);

        dtoList = result.map(fn).stream().collect(Collectors.toList());

    }

    public void makePageResult(Pageable pageRequest){

        this.size = pageRequest.getPageSize();
        this.page = pageRequest.getPageNumber() +1;

        this.tempEnd = (int) ((Math.ceil(page/10.0))*10);

        this.start= tempEnd - 9;
        this.prev = start>1;

        this.end = totalPage > tempEnd? tempEnd : totalPage;
        this.next = totalPage > tempEnd;

        this.pageList = IntStream.rangeClosed(start,end).boxed()
                .collect(Collectors.toList());

    }
}
